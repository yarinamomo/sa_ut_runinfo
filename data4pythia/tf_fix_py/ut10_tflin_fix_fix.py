...
cost = tf.reduce_sum(tf.pow(activation - Y[...,np.newaxis], 2)) / (2 * n_samples)  # L2 loss
...
with tf.Session() as sess:
    ...
    testing_cost = sess.run(tf.reduce_sum(tf.pow(activation - Y[...,np.newaxis], 2)) / (2 * test_X.shape[0]),
                            feed_dict={X: test_X,
