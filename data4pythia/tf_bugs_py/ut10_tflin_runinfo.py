...
# tf Graph Input
X = tf.placeholder(tf.float32, [n_input]) # fix: X = tf.placeholder(tf.float32, [None, n_input])
Y = tf.placeholder(tf.float32) # fix: Y = tf.placeholder(tf.float32, [None])

# Create Model

# Set model weights
W = tf.Variable(tf.zeros([6]), name="weight") # fix: W = tf.Variable(tf.zeros([6, 1]), name="weight")
b = tf.Variable(tf.zeros([1]), name="bias") # fix: b = tf.Variable(tf.zeros([1]), name="bias")
